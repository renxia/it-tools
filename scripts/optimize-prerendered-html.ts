...
const distDir = path.resolve(__dirname, '../dist/it-tools')
...
function optimizeHTMLFile(filePath: string, stats: OptimizationStats, showProgress = true): void {
  try {
    ...
    if (saved > 0) {
      ...
      if (showProgress) console.log(`✓ ${path.relative(distDir, filePath)}: ${(originalSize / 1024).toFixed(2)}KB → ${(optimizedSize / 1024).toFixed(2)}KB (节省 ${savedPercent}%)`)
    }
...
async function optimizePrerenderedHTML(showProgress = true): Promise<void> {
  ...
  console.log(`节省: ${((stats.totalSizeBefore - stats.totalSizeAfter) / 1024 / 1024).toFixed(2)}MB (${(((stats.totalSizeBefore - stats.totalSizeAfter) / stats.totalSizeBefore) * 100).toFixed(2)}%)`)
  console.log('='.repeat(60))
}
