  ...
  <div flex-1>
    <template v-if="!styleStore.isLargeScreen">
      <c-tooltip :tooltip="isMac ? 'Cmd' : 'Ctrl' + '&nbsp;+&nbsp;K'" position="bottom">
        <c-button id="searchBtn" circle variant="text" :aria-label="$t('home.nav.mode')" @click="isModalOpen = true">
          <n-icon size="25" :component="IconSearch" />
        </c-button>
      </c-tooltip>
    </template>
    <template v-else>
      <c-button id="searchBtn" w-full important:justify-start @click="isModalOpen = true">
    ...
    <c-modal v-model:open="isModalOpen" class="palette-modal" overflow-y-auto shadow-xl important:max-w-650px pretty-scrollbar important:pa-12px @keydown="handleKeydown">
      ...
      <div v-for="(options, category) in filteredSearchResult" :key="category">
        <div class="ml-3 mt-3 text-sm font-bold text-primary op-60">
          {{ category }}
        </div>
